...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body style="background-color:#999999; padding:0px; margin:0px;">	
		...
		<ul id="playlists" style="display:none;">
		  ...
		  <li data-source="tvshow" data-playlist-name="{{ $season->tvseries->title }}" data-thumbnail-path="{{asset('images/tvseries/thumbnails/'.$season->thumbnail)}}">
  ...
  data-video-source="[<?php if($slink->url_360 !=""){ ?>{source:'{{ $slink->url_360 }}', label:'360p'}, <?php } ?> <?php if($slink->url_480 !=""){ ?>{source:'{{ $slink->url_480 }}', label:'480p'}, <?php } ?> <?php if($slink->url_720 !=""){?>{source:'{{ $slink->url_720 }}', label:'hd720'}, <?php } ?> <?php if($slink->url_1080 !=""){?> {source:'{{ $slink->url_1080 }}', label:'hd1080'}, <?php } ?>]" @endif data-poster-source="{{asset('images/tvseries/posters/'.$season->tvseries->poster)}}" data-subtitle-soruce="[
		...
		@if($skipads->count()>0)
  		<ul data-ads="">
						...
						@else
						data-source="{{ $skipad->ad_url }}" @endif data-time-start="{{ $skipad->time }}" data-time-to-hold-ads={{ $skipad->ad_hold }} data-thumbnail-source="{{asset('images/tvseries/thumbnails/'.$season->thumbnail)}}" data-link="{{ $skipad->ad_target }}" data-target="_blank"></li>
		</ul>
		@endif
