      ...
      <div class="card" style="padding: 40px;">
        ...
        @if(isset($question) && count($question)>0)
         @foreach($question as $ques)
          <h5 class="card-title">Date: {{str_limit($ques->created_at, 10,' ')}}</h5>
       
        <p class="card-subtitle mb-2 text-muted" style="font-size: 18px; color: #286090">Ques: {{$ques->question}}?</p>
        @if(isset($ques->answer) && !is_null($ques->answer))
        <p style="font-size: 16px;">Reply: {{$ques->answer}}</p><hr>
        @else
